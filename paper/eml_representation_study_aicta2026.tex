...
Condensing multiple nonlinear primitives into a single binary operator streamlines the grammar's vocabulary. However, this syntactic economy comes at a steep spatial cost: it squeezes valid expressions into a remarkably sparse subregion of the candidate space. The high rejection rates under EML are therefore not a temporary algorithmic artifact. Instead, they provide a direct empirical measure of how sparse the valid region actually is within the constrained manifold.
...
This study examined how constrained operator grammars reshape validity-aware symbolic search behavior under fixed evaluation budgets. Structural benchmarks showed that restricting the nonlinear operator set to $\eml(x,y)$ induced systematic structural inflation, increasing tree depth ($4.4\pm0.9$ vs $3.2\pm0.9$ baseline). Rejection analysis demonstrated that this inflation dramatically increased rejection density ($85.9\%$ vs $44.6\%$ baseline) due to depth-limit overflows. Consequently, budget-aware experiments revealed that this sparse validity reduced effective evolutionary throughput, yielding $\approx 67\%$ fewer productive generations under identical nominal evaluation budgets.

These three mechanical effects are strictly coupled: recursive macro-expansion increased the structural footprint of candidate solutions, which in turn increased rejection density under strict validity enforcement, ultimately reducing the number of productive generations achievable under fixed evaluation budgets. We explicitly bound these claims to the conditions of our study. These observations are derived from an exploratory 18-target suite evaluated on a positive-domain fragment, utilizing a deliberately simple evolutionary algorithm without advanced archiving heuristics. Therefore, these results characterize the baseline geometrical distortion induced by the constrained setup, rather than its peak competitive potential. Ultimately, validity-aware accounting may help analyze effective search capacity in other constrained symbolic systems, providing a more transparent metric for evaluating representational costs.
